...
def get_k_perc(length_n_s, length_n_t, length_b_a, length_b_h):
	return 0
	#return length_n_t/length_b_h - length_n_s/length_b_a
...
def get_k(b_a, b_h, i, j, length_b_a, length_b_h):
	α1 = 10.0
	α2 = 2.0
	α3 = 0.2
	α4 = 1.0
	...
	value = α1 * get_k_pos (b_a[i], b_h[j],    b_a[ 0],    b_h[ 0]) + \
			α2 * get_k_perc(b_a[i], b_h[j], length_b_a, length_b_h) + \
...
def get_mapping_cost(b_a, b_h):
	...
	for i in range(len(cost_map)):
		...
		for j in range(len(cost_map[0])):
			...
			cost_map[i][j] = get_k(b_a, b_h, i, j, length_b_a, length_b_h)
	return cost_map
